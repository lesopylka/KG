...
def scaled_hyperboloid (draw_function, scale_x = 1, scale_y = 1, scale_z = 1, time=0.0):
    glPushMatrix()
    scale_x *= math.cos(time * 2 * math.pi / 1000)
    scale_y *= math.sin((scale_x + time) * 2 * math.pi / 1000)
    glScalef(scale_x, scale_y, scale_z)
    draw_function()
    glPopMatrix()
    
def draw_hyperboloid():
...
while True:
    ...
    scaled_hyperboloid(draw_hyperboloid, time=time_passed)
    #draw_hyperboloid()
